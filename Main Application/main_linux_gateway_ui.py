...
def set_config():
    ...
    if request.method == "GET":
        ...
        log_path = BASE / f"is_updated_{file_name}"

        if not config_path.exists():
            print(f"Creating new config file: {config_path}")
            config_path.write_text(json.dumps(new_config or {}, indent=2))
            log_path.write_text(json.dumps(True, indent=2))

            log_update(
                fields=list(new_config.keys()),
                log_file=log_path
            )
        ...
        # Save updated config to dynamic file
        config_path.write_text(json.dumps(current, indent=2))

        # Write update flag to dynamic file
        log_path.write_text(json.dumps(True, indent=2))

        # Log which fields changed
        log_update(
            fields=list(changed_fields.keys()),
            log_file=log_path   # 👈 pass dynamic log file
        )
